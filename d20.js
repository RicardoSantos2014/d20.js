...
(function() {
    ...
    var d20 = {
        ...
        /**
         * Return the mean value of a dice roll
         *
         * @param dice Type of dice to roll, can be represented in various formats:
         *               - a number (6, 12, 42)
         *               - dice syntax (d20, 4d6, 2d8+2)
         * @return Value of mean damage
         */
        minimumDamage: function(dice) {
            var numberOfDice,
                numericModifier = 0,
                result,
                match,
            ...
            if (typeof dice == 'string') {
                ...
                if (match) {
                    if (match[1]) {
                        numberOfDice = parseFloat(match[1]);
                    }
                    if (match[2]) {
                        dice = parseFloat(match[2]);
                    }
                    if (match[3]) {
                        modifiers = match[3].match(/([+-]\s*\d+)/g);
                        for (var i = 0; i < modifiers.length; i++) {
                            numericModifier += parseFloat(modifiers[i].replace(/\s/g, ''));
                        }
                    }
                } else {
                    parseFloat(dice);
